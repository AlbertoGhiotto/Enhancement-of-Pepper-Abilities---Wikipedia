...
def checkWiki(keyword):
    print(keyword)
    try:
        print("Checkwiki->1-try ")
        wikipedia_mediawiki = MediaWiki()  # check if exists a page corresponding to the user input
        wikiPage = wikipedia_mediawiki.page(keyword)
        print(wikiPage.summary)
        return [True, wikiPage]  # return the user input as the keyword
    except:
        try:
            print("Checkwiki->2-try without autosuggest")
...
def keywordExtraction():
    while True:
        ...
        else:                                           # case in which the user inputs only the keyword
            if(checkWiki(keyword_sentence)[0] == False):
                continue
            else:
                wikiPage = checkWiki(keyword_sentence)[1]
                return [wikiPage, keyword_sentence]

        single_keyword = keyword_sentences[1].split("?")
        if(checkWiki(single_keyword)[0] == False):
            continue

        wikiPage = checkWiki(single_keyword)[1]

        return [wikiPage, single_keyword]
...
while True:
    ...
    # while True:
        ...
        # except:
        #     try:
        #         print("Trying without auto suggest")
        #         wikiPage = wikipedia_mediawiki.page(keyword, auto_suggest=False)
        #         break
        #     except:
